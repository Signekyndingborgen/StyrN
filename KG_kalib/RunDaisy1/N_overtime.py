...
for root, dirs, filenames in items:
    for d in dirs:
        ...
        rgN=rg['Fixated', NLeaf	NDead	NStem	NSOrg	NRoot.sum.axis()
        ax=plt.subplot(4,3,index)
        index+=1
        plt.scatter(rg.index, (rg['LAI']/rg['WLeaf']),  marker='o', c='black', s=5, label= 'grass')
        plt.scatter(wc.index, (wc['LAI']/wc['WLeaf']),  marker='x', c='red', s=5, label = 'clover')
        plt.legend()
#        plt.legend(('time', 'LAI/N_leaf'))
        plt.title(d, position = (0.6, 0.8), fontweight="bold", fontsize=10)
        ax.set(ylabel=('LAI/leaf'))
        plt.show()
        plt.tight_layout()

fig.savefig("LAI/leaf_v1.pdf", bbox_inches='tight')      
