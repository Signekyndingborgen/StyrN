...
def write_columns(path):
    init = getSOMspinup(r'../RunSpinup')
    initlevels = ['Medium']
    
    rota = pd.DataFrame(pd.read_excel(xlfile,sheet_name= 'RotationsKSB'))
    rota.index = rota['ID']
    ...
    if not os.path.isdir(path):
        os.makedirs(path)
    template = DaisyModel(os.path.join(path, '../Common/Scenarier_v8.dai'))
    template2 = DaisyModel(os.path.join(path, '../Common/Def_columns.dai'))   # SoilJB1_4_5_OM.dai
    weather = SoilClimate['Climate'][0:1].tolist()
