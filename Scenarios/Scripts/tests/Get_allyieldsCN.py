...
grain = ['SB', 'Winter Wheat JG','Vinterbyg','Rug','Winter Rape PA','Spring Wheat',
         'Rug', 'Froegraes', 'Potato SavaFigaro','Sugar Beet','Pea', 'Potato; FertOrgaNic']
silo = ['Ryegrass', 'Wclover', 'Silomajs', 'SB-green', 'Graes'] 

def get_allYieldsCN(path):
    items = os.walk(path)
    allresults={}
            ...
            if os.path.isfile(os.path.join(root, d, "harvest.dlf")):
                ...
                for cropname in silo:
                    if cropname in DMS.groups.keys():
                        dm = DMS.get_group(cropname).sum(axis=1)
                        cropyield[cropname]= list(dm.resample('Y').sum())
                        n = NS.get_group(cropname).sum(axis=1)
                        cropyield[cropname]= list(n.resample('Y').sum())
