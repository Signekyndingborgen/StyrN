...
for ro in rotations:    
    dict2 = {k:value for k, value in dSOMperkey.items() if split_unique_name(k)['Rotation']== ro}
    df =pd.DataFrame.from_dict(dict2, orient='index') 
    means = df.mean(axis = 0)
    SOMmeanperRot[ro] = means   
SOMmean =SOMmeanperRot.transpose()
SOMmean.columns = ['dSOC', 'dSON']
...
fig.savefig("Mean dSON per rotation.pdf", bbox_inches='tight')    
